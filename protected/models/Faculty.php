...
class Faculty extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, class, postcode, state, country', 'required'),
			array('postcode', 'numerical', 'integerOnly'=>true),
			array('name, class, state, country', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, class, postcode, state, country', 'safe', 'on'=>'search'),
		);
	}
